...
async function detection(job:any,rows:any[]){
  const p=job.parameters||{};
  const rw=+p.volatility_window||30;
  const vw=+p.volume_window||30;
  const sig=+p.return_threshold_sigma||3;
  const vm=+p.volume_multiple||5;
  const min=+p.minimum_history||30;
  const spacing=+p.event_spacing_days||0;
  ...
  for(const[symbol,g]of groups){
    ...
    for(let i=Math.max(rw,vw,min);i<g.length;i++){
      const ar=n(g[i].Abnormal_Return);
      const vol=n(g[i].Vol);

      if(ar==null||vol==null)continue;

      const histR=g
        .slice(i-rw,i)
        .map(x=>n(x.Abnormal_Return))
      ...
      if(histR.length<rw||histV.length<vw)continue;

      const s=std(histR);
      const mv=mean(histV);
      const mult=mv?vol/mv:0;
      const rs=s?Math.abs(ar)/s:0;
      ...
      if(
        ...
        const preCar=preReturns.reduce((sum,x)=>sum+x,0);
        const positivePreDays=preReturns.filter(x=>x>0).length;

        const preCarScore=Math.min(
          40,
          Math.max(0,Math.abs(preCar)/0.10*40)
        );

        const volumeScore=Math.min(
          25,
          Math.max(0,(mult-1)/9*25)
        );

        const sigmaScore=Math.min(
          20,
          Math.max(0,(rs-3)/7*20)
        );

        const consistencyScore=preReturns.length
          ? positivePreDays/preReturns.length*15
          : 0;
        ...
        out.push({
          ...
          symbol,
          event_date:g[i].Date,
          open_price:n(g[i].Open),
          high_price:n(g[i].High),
          low_price:n(g[i].Low),
          close_price:n(g[i].Close),
          volume:vol,
          turnover:n(g[i].Turnover),
          transactions:n(g[i]['Trans.']),
          previous_close:n(g[i]['Prev. Close']),
          stock_return:n(g[i].Return),
          market_return:n(g[i].Market_Return),
          abnormal_return:ar,
          rolling_volume_mean:mv,
          rolling_return_std:s,
          volume_multiple:mult,
          return_sigma:rs,
          severity_score:Math.min(100,mult*2.5+rs*5),
          insider_score:Number(insiderScore.toFixed(2)),
          event_year:new Date(g[i].Date).getFullYear(),
  ...
  const{data:run,error:re}=await sb
    ...
    .insert({
      project_id:job.project_id,
      run_name:`Detection ${new Date().toISOString()}`,
      run_type:'abnormal_event_detection',
      parameters:{
        ...p,
        insider_score_method:'pre_5_day_car_volume_sigma_consistency_v1'
      },
      status:'completed',
      records_processed:rows.length,
      events_detected:out.length
    })
    .select('id')
    .single();
  ...
  for(let i=0;i<out.length;i+=1000){
    const{error}=await sb
      ...
      .upsert(out.slice(i,i+1000),{
        onConflict:'project_id,symbol,event_date'
      });
